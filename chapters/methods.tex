...
At the beginning of the function call, the target vector data is uploaded as Fusion Table illustrated with the arrows number 1. 
The group of processes with the number 2, describes the integration of R and Python and the method Python code is executed from R. The processes with number 3 in, represents the exchange of arguments, or,  how parameters are passed from R to Python. The arrows with the number 4 illustrate the structure of the Python code as executable modules. The processes with the number 5 show the communication of the Python client library and EE while the arrows with number 6 describe the approach to send process info from EE to R. The arrows with number 7 illustrate the import of Fusion Table, the computation and the export of the processed data is and finally the processes with number 8, describes the access from R to Google Drive and the following download and import of the final data into R.
...
The EE is accessed with the EE client library available for Python. To access the EE from R it's necessary to execute Python from R. The integration should enable to pass arguments from R to Python and execute Python code from R. Instead of using a Python wrapper for R like the rPython package, earthEngineGrabR utilizes a simple command line or terminal to execute one language from the other, illustrated by the arrows with the number 1. The \texttt{ee\_grab()} function defined in the R script functions.R, invokes a system call by utilising the command line that executes the python execution script (execution.py) and simultaneously, indicated with the number 2, passes the parameters, to the execution scripts by using a flat file. 
...
At the moment the script is executed, this description is sent to the EE servers through a REST API, in figure \ref{processingFlow} indicated as arrow with the number 4. REST is a web service often used to request and modify data on a server through a Hypertext Transfer Protocol (HTTP).
